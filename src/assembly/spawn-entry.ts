 ...
 * spawn watchdog entry 路径解析 + bundle 判别（phase 1436 抽象、phase 72 自
 * foundation/paths.ts 整迁 → L6 Assembly 真业务 owner）。
 *
 * 业务：装配期决定 watchdog-entry.js 物理路径、跨 bundled (tsup
 * 平铺至 dist/) vs unbundled (tsc 保 src 层级 dist/assembly/) 模式自动判别。
 *
 * 模式判别基础：basename 比较纯路径结构、无 fs 依赖。
 *
 * 散落历史（phase 1436 之前）：caller 各自手算路径、多种 fallback 层数 + exists
 * 判定写法。tsup code-split 平铺策略一改即全炸（toolprotocol-rechecker /
 * messaging-auditor / tools-auditor daemon spawn MODULE_NOT_FOUND 实证）。
 *
 * signature 保 `(_fs?: FileSystem)`：caller 调用形式与既有 `existsSync` 风格一致、
 * 未来若 entry 路径源迁回 user fs（罕见）签名不破。当前实现不消费 fs 参数。
 *
 * phase 1284：daemon entry 路径解析归位 Daemon 真 owner（`daemon/entry-resolver.ts`），
 * 本文件只保留 Watchdog resolver。
 *
