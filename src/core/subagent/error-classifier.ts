...
export function classifyAndAuditError(opts: ClassifyErrorOptions): void {
  const { error, safeSwWrite, sink, timeoutMs } = opts;
  ...
  if (error instanceof ToolTimeoutError) {
    ...
    sink.turnInterrupted({ cause: 'turn_timeout', ms: timeoutMs });
    ...
    if (error.reason.kind === 'idle_timeout') {
      ...
      sink.turnInterrupted({ cause: 'idle_timeout', ms: error.reason.ms });
    ...
    sink.turnInterrupted({ cause: 'external', ...(causeType ? { type: causeType } : {}) });
  } else {
    safeSwWrite({ ts: Date.now(), type: SUBAGENT_EVENTS.TURN_ERROR, error: errMsg });
    sink.turnError({ error: errMsg });
  }
}
