 ...
 *     读 stream），返配对比两次工厂简洁；代价=少数只需 audit 的调用方会拿到冗余 fs（可忽略，
 *     NodeFileSystem 构造零成本）
 *   - 文件名 shared.ts 的 meta 词合规性：无 CLI 层子领域差异，"shared" 即 "CLI 层共享"，不违反
 *     "同一概念同一名字"（若未来出现分化，拆分至 cli-factories.ts / cli-context.ts）
 */
