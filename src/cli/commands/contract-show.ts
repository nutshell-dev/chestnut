...
export async function contractShowCommand(deps: { fsFactory: (baseDir: string) => FileSystem }, clawId: string, contractId?: string): Promise<void> {
  ...
  const manager = new ContractSystem({ clawDir, clawId: makeClawId(clawId), fs: clawFs, audit, toolRegistry: createToolRegistry(), fsFactory: deps.fsFactory, notifyClaw: (targetClawId, message) => notifyClaw(clawFs, chestnutRoot, MOTION_CLAW_ID, targetClawId, message, audit) });
