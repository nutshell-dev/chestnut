/**
 * shutdown invariants — mechanical merge of the following source files
 * (no assertion logic changed):
 *  - watchdog-shutdown-guard.test.ts
 *  - handler-idempotent-install.test.ts
 */
...
describe('watchdog-shutdown-guard', () => {
  ...
  describe('watchdog signal handler lifecycle (phase 994 A.1)', () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(readWorkspaceWatchdogConfig).mockReturnValue({
        interval_ms: 100, heartbeat_stale_timeout_ms: 180_000,
      });
...
describe('handler-idempotent-install', () => {
  const fsFactory = (dir: string) => new NodeFileSystem({ baseDir: dir });
