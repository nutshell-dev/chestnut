...
describe('contract create graceful format (phase 67)', () => {
  ...
  it('handleCliError for ContractValidationError already_exists returns 1 with guidance', () => {
    const stderrSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const err = new ContractValidationError('id', 'already_exists', 'contract id "new-id" already exists in active', { contractId: 'new-id' });
    ...
    expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('yaml validation failed'));
    expect(stderrSpy).toHaveBeenCalledWith('  field:    id');
    expect(stderrSpy).toHaveBeenCalledWith('  kind:     already_exists');
