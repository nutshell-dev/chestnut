...
describe('Builtin Tools', () => {
  ...
  describe('read tool', () => {
    ...
    // Phase 258: base read tool 仅同 claw、error 不含 cross-claw Tip
  ...
  describe('ls tool', () => {
    ...
    // Phase 258: base ls tool 仅同 claw、error 不含 cross-claw Tip
    it('should return structured error for non-existent path', async () => {
  ...
  describe('exec tool', () => {
    ...
    it('should include node bin dir in PATH for subprocess', async () => {
      ...
      expect(result.success).toBe(true);
      // 子进程 PATH 应包含 node 可执行文件所在目录（chestnut 命令所在位置）
    ...
    it('clawDir 不存在时返回 ENOENT 错误（success:false）', async () => {
      const missingDirCtx = new ExecContextImpl({
        ...
        workspaceDir: path.join(tempDir, 'nonexistent-dir-xyz', 'clawspace'),
        syncDir: path.join(tempDir, 'nonexistent-dir-xyz', 'tasks', 'sync'),
        profile: 'full',
        fs: mockFs,
        fsFactory: (dir: string) => new NodeFileSystem({ baseDir: dir }),
