...
describe('ReAct Loop', () => {
  ...
  it('should handle multiple tool_use blocks in one response (flushes previous tool_use)', async () => {
    ...
    (mockLLM.stream as ReturnType<typeof vi.fn>)
      .mockImplementationOnce(async function* () {
        ...
        yield { type: 'tool_use_delta', toolUse: { id: 'id-2', name: 'search', partialInput: '{"text":"foo"}' } };
        yield { type: 'done' };
      })
      .mockImplementationOnce(async function* () {
        yield { type: 'text_delta', delta: 'Done' };
  ...
  it('should execute readonly sync tools in parallel via executeParallel', async () => {
    ...
    const llm = {
      stream: async function* () {
        ...
        if (step === 1) {
          ...
          yield { type: 'tool_use_delta', toolUse: { id: 'id2', name: 'search', partialInput: '{"text":"q"}' } };
          yield { type: 'done' };
        } else {
    ...
    expect(batch[1].args).toEqual({ text: 'q' });
