...
describe('EventLoop phase 1390 Step B: onBlockedTerminal fail-fast', () => {
  let agentDir: string;
  let inboxPendingDir: string;
  const fsFactory = (dir: string) => new NodeFileSystem({ baseDir: dir });
  ...
  beforeEach(() => {
    // eslint-disable-next-line chestnut-custom/no-bare-tempdir-in-tests
    agentDir = path.join(os.tmpdir(), `event-loop-1390-${randomUUID()}`);
    require('fs').mkdirSync(agentDir, { recursive: true });
    inboxPendingDir = path.join(agentDir, 'inbox', 'pending');
    require('fs').mkdirSync(inboxPendingDir, { recursive: true });
  });
  ...
  function makeAudit(): AuditLog & { entries: [string, ...(string | number)[]][] } {
    const entries: [string, ...(string | number)[]][] = [];
    return {
      entries,
      write: (type: string, ...cols: (string | number)[]) => { entries.push([type, ...cols]); },
  ...
  function makeRuntime(
    ...
    fingerprint: string,
    reactiveTrimResult?: unknown,
  ) {
    const processTurn = vi.fn().mockResolvedValue(processTurnResult);
    const nackHandles = vi.fn().mockResolvedValue(undefined);
    const reactiveTrim = reactiveTrimResult !== undefined
      ? vi.fn().mockResolvedValue(reactiveTrimResult)
  ...
  it('invalid_request 进入 blocked 时 onBlockedTerminal 被调用一次、state 携带 reason+fingerprint', async () => {
    vi.useFakeTimers();
    const audit = makeAudit();
    const onBlockedTerminal = vi.fn().mockResolvedValue(undefined);
    const err = new LLMInvalidRequestError('openai', 'invalid_unicode');
    const { runtime } = makeRuntime(makeTurnResult('failed', { error: err }), 'fp-invalid');
    ...
    expect(onBlockedTerminal).toHaveBeenCalledTimes(1);
    const state = onBlockedTerminal.mock.calls[0][0];
    expect(state).toMatchObject({
      reason: 'invalid_request',
      requestFingerprint: 'fp-invalid',
  ...
  it('permanent_provider_error 进入 blocked 时 onBlockedTerminal 被调用一次', async () => {
    vi.useFakeTimers();
    const audit = makeAudit();
    const onBlockedTerminal = vi.fn().mockResolvedValue(undefined);
    const err = new LLMAuthError('openai', 401, 'bad key');
    const { runtime } = makeRuntime(makeTurnResult('failed', { error: err }), 'fp-auth');
    ...
    expect(onBlockedTerminal).toHaveBeenCalledTimes(1);
    expect(onBlockedTerminal.mock.calls[0][0]).toMatchObject({
      reason: 'permanent_provider_error',
      requestFingerprint: 'fp-auth',
    });
  });

  it('retry_exhausted（context_exceeded + retry 已耗尽）进入 blocked 时 onBlockedTerminal 被调用', async () => {
    vi.useFakeTimers();
    const audit = makeAudit();
    const onBlockedTerminal = vi.fn().mockResolvedValue(undefined);
    const ctxErr = new LLMContextExceededError('p', 400, 'context length exceeded');
    const { runtime } = makeRuntime(makeTurnResult('failed', { error: ctxErr }), 'fp-re');
    // 预置 retry count 到 max
    const statusDir = path.join(agentDir, 'status');
    require('fs').mkdirSync(statusDir, { recursive: true });
    require('fs').writeFileSync(
      path.join(statusDir, 'llm-retry-state.json'),
      JSON.stringify({ schema_version: 1, llmRetryCount: LLM_MAX_RETRIES, llmRetryDelayMs: 10, llmRetryPending: false }),
    );
    ...
    expect(onBlockedTerminal.mock.calls[0][0]).toMatchObject({
      reason: 'retry_exhausted',
      requestFingerprint: 'fp-re',
    });
  });

  it('no_progress / policy_conflict 进入 blocked 时 onBlockedTerminal 均被调用（trim outcome 双分支）', async () => {
    for (const status of ['no_progress', 'policy_conflict'] as const) {
      vi.useFakeTimers();
      // 每分支独立 agentDir（blocked state 落盘）；用主 beforeEach 的 agentDir 会跨分支污染
      // eslint-disable-next-line chestnut-custom/no-bare-tempdir-in-tests
      const dir = path.join(os.tmpdir(), `event-loop-1390-${status}-${randomUUID()}`);
      require('fs').mkdirSync(path.join(dir, 'inbox', 'pending'), { recursive: true });
      const audit = makeAudit();
      const onBlockedTerminal = vi.fn().mockResolvedValue(undefined);
      const ctxErr = new LLMContextExceededError('p', 400, 'context length exceeded');
      const { runtime } = makeRuntime(makeTurnResult('failed', { error: ctxErr }), `fp-${status}`, {
        status,
        before: 1000,
        after: 1000,
        reason: 'x',
      ...
      const eventLoop = new EventLoop({
        ...
        clawId: 'test-claw',
        audit,
        inbox: { pendingDir: path.join(dir, 'inbox', 'pending'), fallbackTimeoutMs: 50 },
        onBlockedTerminal,
      });
  ...
  it('onBlockedTerminal 抛错 → BLOCKED_TERMINAL_FAILED audit、gate 语义不被阻塞', async () => {
    vi.useFakeTimers();
    const audit = makeAudit();
    const onBlockedTerminal = vi.fn().mockRejectedValue(new Error('cancel boom'));
    const err = new LLMInvalidRequestError('openai', 'invalid_unicode');
    const { runtime } = makeRuntime(makeTurnResult('failed', { error: err }), 'fp-throw');
    ...
    expect(onBlockedTerminal).toHaveBeenCalledTimes(1);
    expect(audit.entries.some(
      e => e[0] === EVENTLOOP_AUDIT_EVENTS.BLOCKED_TERMINAL_FAILED
        && e.some(c => String(c).includes('cancel boom')),
    )).toBe(true);

    // 后续 tick 仍然 gate blocked（gate 语义不被异常破坏）
    const run2 = eventLoop.run();
    await vi.advanceTimersByTimeAsync(100);
    await run2;
    expect(audit.entries.filter(e => e[0] === EVENTLOOP_AUDIT_EVENTS.CONTEXT_BLOCKED_GATE).length).toBe(1);
  ...
  it('未注入 onBlockedTerminal 时 blocked 路径行为保持不变（无 crash）', async () => {
    vi.useFakeTimers();
    const audit = makeAudit();
    const err = new LLMInvalidRequestError('openai', 'invalid_unicode');
    const { runtime } = makeRuntime(makeTurnResult('failed', { error: err }), 'fp-none');
