...
const parserSource = readFileSync(
  new URL('../../../src/foundation/llm-provider/openai-sse-parser.ts', import.meta.url),
  'utf8',
);

describe('LLMProvider OpenAI stream parse error callback surface', () => {
  it('keeps the callback type local while preserving the parser contract', () => {
    ...
    expect(parserSource).toMatch(/export\s+async\s+function\*\s+parseSSEStream\s*\(/);
