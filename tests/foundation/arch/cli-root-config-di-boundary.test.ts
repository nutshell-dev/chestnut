 ...
 * 4. CLI production 下 `assembly/config/config-load.js` importer 精确为 22 文件
 *    migration baseline——只防新增与意外删除，不批准永久存在；后续每个命令族
 *    治理 phase 必须同步递减本清单；
 * 5. clawExists 不得回到 router；
 * 6. phase 1324：claw read/ls 零 Assembly config internal、共享 ClawCommandDeps。
 */
...
describe('phase 1301: remaining deep-caller migration baseline', () => {
  it('config-load.js importer 精确为 22 文件路径集合（phase 1324 迁出 read/ls）', () => {
...
describe('phase 1324: clawspace read/ls 共享窄 deps 边界', () => {
  it('read/ls 零 assembly/config/**、type-import 共享 deps、参数为 ClawCommandDeps', () => {
    for (const file of CLAWSPACE_COMMANDS) {
      const text = read(path.join(CLI_ROOT, 'commands', file));
      expect(importSpecifiers(text).filter((s) => s.includes('assembly/config/'))).toEqual([]);
