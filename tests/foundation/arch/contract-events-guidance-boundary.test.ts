 ...
 *  - 唯一 consumer（Assembly contract-events composer）必须经 owner decoder 取
 *    typed refs，不得再出现 raw owner dialect（source_claw / contract_id /
 *    problem_pairs / JSON.parse / CSV split / ID regex）；
...
const COMPOSER = path.join(srcRoot, 'assembly', 'guidance', 'composers', 'contract-events.ts');
...
describe('phase 1261 Step B: contract events guidance owner boundary', () => {
  ...
  it('composer 不得出现 raw owner dialect（wire keys / JSON.parse / CSV split / ID regex）', () => {
    const text = fs.readFileSync(COMPOSER, 'utf8');
