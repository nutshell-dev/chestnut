 ...
 *
 * 锁定已迁 4/4 typed binding 纵向切片（claw_inactivity / claw_outbox_summary / contract_events / contract_cancelled；phase 1380: claw_crashed 退场）的边界（总览反向验收 2/3/4）：
...
describe('phase 1263 Step C + phase 1264 Step A + phase 1265/1266/1267 Step A: cli guidance typed boundary', () => {
  ...
  it('反向 fixture：scanner 能检出违反形态、不误放合法形态', () => {
    ...
    const inactivityRe = bindingForbiddenRe(CLI_GUIDANCE_BINDINGS[0]), outboxRe = bindingForbiddenRe(CLI_GUIDANCE_BINDINGS[1]), eventsRe = bindingForbiddenRe(CLI_GUIDANCE_BINDINGS[2]), cancelledRe = bindingForbiddenRe(CLI_GUIDANCE_BINDINGS[3]);
    expect(inactivityRe.test("return { text: 'x' };")).toBe(true);
    expect(inactivityRe.test('`chestnut claw ${id} daemon`')).toBe(true);
    expect(inactivityRe.test("'To inspect: ' + cmd")).toBe(true);
    expect(outboxRe.test("'查看具体内容' + cmd")).toBe(true);
    expect(inactivityRe.test('renderCliGuidanceDocument(doc)')).toBe(true);
    expect(inactivityRe.test('renderClawInvocation(id, cmd)')).toBe(true);
    ...
    expect(inactivityRe.test("defineCliGuidanceBinding({ type: 'claw_inactivity', decode, toDocument })")).toBe(false);
    expect(outboxRe.test("return { lines: [{ label: 'restart', action }] };")).toBe(false);
