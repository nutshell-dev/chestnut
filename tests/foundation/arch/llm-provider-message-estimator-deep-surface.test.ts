...
describe('LLMProvider single-message estimator deep surface', () => {
  it('keeps estimateMessageTokens local behind estimateMessagesTokens', () => {
    ...
    expect(estimatorSource).toMatch(/(?:^|\n)(?:export\s+)?function\s+estimateMessageTokens\s*\(msg:\s*Message\):\s*number\s*\{/);
    expect(estimatorSource).toMatch(/export\s+function\s+estimateMessagesTokens\s*\(messages:\s*readonly\s+Message\[\]\):\s*number\s*\{/);
