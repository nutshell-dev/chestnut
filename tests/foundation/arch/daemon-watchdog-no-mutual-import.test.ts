 ...
 *
 * 期望状态（phase 444 闭环）：
 *   - src/daemon/* 0 import src/watchdog/*（daemon-entry.ts 除外）
 *   - src/watchdog/* 0 import src/daemon/*（watchdog-entry.ts 除外）
...
describe('daemon ↔ watchdog no mutual module import ratchet (phase 493)', () => {
  it('src/daemon/* contains 0 imports of src/watchdog/*', () => {
    ...
    const cmd = `grep -rEn "from ['\\\"][^'\\\"]*watchdog/" ${srcDaemonRoot} --include='*.ts' --exclude='daemon-entry.ts' || true`;
  ...
  it('src/watchdog/* contains 0 imports of src/daemon/*', () => {
    ...
    const cmd = `grep -rEn "from ['\\\"][^'\\\"]*daemon/" ${srcWatchdogRoot} --include='*.ts' --exclude='watchdog-entry.ts' || true`;
    const out = execSync(cmd, { encoding: 'utf8' });
    expect(out.trim()).toBe('');
  });
});
