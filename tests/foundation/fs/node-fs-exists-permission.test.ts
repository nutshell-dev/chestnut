...
describe('NodeFileSystem — exists PermissionError signal (P1.5 phase 611)', () => {
  ...
  it('throws PermissionError when probing absolute path', async () => {
    // 修前：catch all 返 false / probing /etc/passwd 静默返 false
    // 修后：PermissionError 抛 / 安全 signal 保留
    await expect(fs.exists('/etc/passwd')).rejects.toThrow(PermissionError);
  });
