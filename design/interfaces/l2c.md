...
// 注：InboxMessage type 物理隔离在 src/types/messaging.ts（cross-module shared types / phase344 D 落地 / 不在本节 export 重复定义 / 跨模块共享 schema 单源）
```
...
- `InboxReader` ctor 接 3 dirs (pendingDir / doneDir / failedDir) + fs + audit / `createInboxReader` factory 接 (fs, audit, baseDir) 内部派生 3 dirs（默认 inbox/{pending,done,failed} 子目录约定 / 简化 caller 装配 / caller 需自定义 dir 命名时直接用 ctor）
...
- `l6_assembly`（DI，装配期扫描加注入 + 注册 skill 工具到 L2 Tools）
- `l5_runtime`（DI，每轮 dispatch 上下文摘要 / ContextInjector 内部组件）
- `l4_task_system`（DI，任务执行期加载 skill 内容）
- ~~`l4_evolution_system`（DI，retro 完成后调 reload 触发 rescan）~~ ⚠ phantom (ε 2026-05-07 / l2c §G2 closure / 详 modules/l4_evolution_system.md §A.dead-1)：retro 实然走 per-execution lazy load 模式（dispatch + retro-scheduler 各 createSkillSystem 临时 create / 不经 main skillRegistry reload）/ EvolutionSystem 不实质 dep main skillRegistry / 推 r+1 code phase 删 dead field
- `l3_sub_agent`（DI，子代理装配期上下文摘要）

> 注：skill 工具 phase 442 后物理迁 SkillSystem own（同 phase 360 done + phase 416 memory_search 模板）/ 不再是独立 `l2_skill_tool` 消费方 / 工具 own 在本模块内（详 modules §A.tool-1 closed + §10 工具通道）。
...
export class SkillSystem {
  ...
  formatForContext(): string;                          // 上下文摘要生成（per arch §14 表 2 + 表 1 耦合窄化点）/ 3 caller: ContextInjector + retro-scheduler + dispatch
}
...
// 工厂（phase370 闭环：skillsDir 必填 / 删 default value / caller 全显式传 const ref）
export function createSkillSystem(
  fs: FileSystem,
  skillsDir: string,     // phase370 必填 / 不再 default
  audit?: AuditLog,
): SkillSystem;
```
...
- `register(skillDir)` 手动注册单个 / 应然 method 但当前 0 caller 真调用（实然 retro skill 走 per-execution lazy load / 不经 main instance register / 详 §G2 closure ε 2026-05-07）
...
**应然权威**：模块名 `SkillSystem` align architecture.md §14 + 表 1 权威（实然 code 命名 + 物理位置 drift 见 modules §A）。

**Design-gap（业务决策性 / 用户拍板候选 / r65 cross-doc audit 浮出 / 详 modules/l2_skill_system.md §B）**：

| Gap | 描述 | 候选 |
|---|---|---|
| **L2c.G1** 上下文摘要生成能力归属 | arch 表 2 SkillSystem 列「上下文摘要生成」+ 表 1 耦合「上下文摘要格式化作为耦合窄化点」 / interfaces 0 暴露对应 method（仅 loadAll/register/listMeta/loadFull）/ 实然可能在 Runtime ContextInjector 自治 format | ✅ **closed by ε 决策（2026-05-07）**：spec 修正 / interfaces 加 `formatForContext(): string` method 暴露 / 实然 SkillSystem own format（`registry.ts:170` / 3 caller: ContextInjector + retro-scheduler + dispatch）/ arch 表 2 + 表 1 耦合描述实然 align（不动）/ 反对 α 名错（method 实然名 `formatForContext` / 不是 `getContextSummary`）/ 反对 β 反 M#1+M#3（实然 SkillSystem own format）/ 反对 γ 反 M#11（spec stale 不显式登记）|
| **L2c.G2** reload 触发 rescan 描述精度 | arch 表 2 列「reload 触发 rescan」/ interfaces 仅 `register(skillDir)` 增量 / 注「register 用于 retro skill 沉淀后调（EvolutionSystem.reload 间接经此）」/ reload 是 EvolutionSystem 业务概念 vs SkillSystem 原语描述错位 | ✅ **closed by ε 决策（2026-05-07）**：framing 推翻（同 phase 458 STALE + l6_watchdog A.spec-2 同型）/ Path #1 实测核浮出：retro skill 实然走 **per-execution lazy load** 模式（dispatch + retro-scheduler 各 `createSkillSystem(...DISPATCH_SKILLS_DIR)` 临时 create instance / 不经 main skillRegistry instance）/ retro skill 写到 `clawspace/dispatch-skills/` / 与 main `SKILLS_DIR_DEFAULT` 隔离 / **EvolutionSystem.skillRegistry field declared 但 0 真调用 = dead field**（同 phase 426 ContractManager.retroScheduler 同型）/ `skill_system_reload_triggered` + `RETRO_RELOAD_FAILED` audit events 实然 0 写 = phantom / 'reload_failed' EvolutionError code dead status / 反对 α 不全（仅改 arch 描述未清 dead field 群体）/ 反对 β 反 M#8（reload() method = over-engineering / 0 caller 需求）/ 反对 γ 反 M#11（dead intent 群体不清）/ design only + r+1 code phase dead intent cluster 顺手清 |
...
- `register` 装配期一次注入 / `unregister` 仅测试场景用（生产路径运行期不动注册表 / 状态稳定）
- `get` / `has` / `getAll` / `getForProfile` 同步查询 / 不影响状态
- `formatForLLM(tools)` 转 LLM API 接受的 tool_use schema 形态（input_schema 字段名 / 与 internal Tool.schema 字段名区分 / Anthropic API convention）
...
- 调用前后 audit 写 L2 AuditLog（事件命名空间归本模块 / events 见 modules）

**应然权威**：模块名 `Tools` align architecture.md §15 + 表 1 权威（实然物理位置 cross-layer drift 见 modules §A）。
...
注：`Tool` 来自 [L2 ToolProtocol](./l2b.md#toolprotocol-type-only)（cross-ref interfaces/l2b.md）。**应然 silent on `FileToolError` 错误类**（失败返结构化 ToolResult `{ success: false, error }` / 不抛异常）/ 实然 drift 见 modules §A。

**使用语义**：

- `createFileTools(options)` factory 装配期返 4 工具 (read / write / search / ls) 数组 / Assembly 显式 register 进 ToolRegistry
- `FileToolOptions.allowedRoots` 装配期注入路径根（如 `<clawDir>/clawspace/`）/ 运行期不变 / 越界守护用此 list 检查
...
export interface CommandToolDeps {
  processExec?: ProcessExec;            // L1 ProcessExec 接口（cross-ref interfaces/l1.md / 装配期注入）
...
注：`Tool` 来自 [L2 ToolProtocol](./l2b.md#toolprotocol-type-only)（cross-ref interfaces/l2b.md）/ `ProcessExec` 来自 [L1 ProcessExec](./l1.md#processexec-capability-direct--di)（cross-ref interfaces/l1.md）。本模块不抛 CommandToolError class — blacklist 命中 + 子进程非零退出都返 ToolResult `{ success: false, error }` / 不抛异常。
