...
## phase 683 — barrel M#9 hygiene cluster + dead field cleanup + abort helper barrel completeness（r91 B fork）

**Step E（dead field cleanup / Tier 1）**：
- `RuntimeOptions.maxConcurrentTasks` + `RuntimeOptions.subagentMaxSteps` 删（6 files / -8 行 / 0 行为差）— **closed by phase 683 Step E.1+E.2** / `design/modules/l5_runtime.md` §7.A 同步
- `ToolExecutor.profile` private field + `ToolExecutorOptions.profile` 删（4 files / -4 行 / 0 行为差）— **closed by phase 683 Step E.3** / `design/modules/l2_tools.md` §7.A 同步

**Step F（barrel M#9 cross-module import normalize / Tier 2）**：
- 11 barrel 分组 / 20+ files / 30+ import line / 0 行为差 — **closed by phase 683 Step F**
- messaging（InboxReader/InboxEntry/OutboxWriter）/ stream（StreamWriter/StreamLog/StreamEvent/STREAM_FILE）/ file-watcher（Watcher）/ tools（ToolRegistry/IToolExecutor/ExecContextImpl/ToolExecutor/TOOL_PROFILES）/ llm-orchestrator（LLMOrchestratorConfig/LLMCallOptions）/ skill-system（SkillSystem/SkillMeta）/ contract（ContractSystem）/ process-manager（ProcessManager）/ gateway（Gateway/createGateway/createAskUserTool）/ async-task-system（StreamLog）/ agent-executor（runReact）
- 例外保留：同模块内 sub-path（orchestrator.ts:29 `../llm-provider/abort-helper.js`）/ jobs sub-path / barrel 未 re-export symbol（fs/types.js / tool-protocol/caller-type.js / llm-provider/presets.js / tools/tool-names.js 等）

**Step G（A.6 abort helper barrel completeness）**：
- `llm-provider/index.ts` 加 export `classifyFetchAbortError` + `makeExternalAbortError` + `AbortReason`（3 symbol）/ `subagent/agent.ts:15-16` 改 import 路径走 barrel — **closed by phase 683 Step G** / `design/modules/l1_llm_provider.md` §7.A 同步
- `orchestrator.ts:29` 同模块内保留 sub-path（例外规则）
