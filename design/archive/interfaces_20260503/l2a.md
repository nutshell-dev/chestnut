...
**应然权威**：interface 名 `AuditLog` align architecture.md §6 + 表 1 权威。**实然 code 用 `Audit` 名（短名 / src/foundation/audit/index.ts:23）**：登记 code drift / 详 [modules/l2_audit_log.md](../modules/l2_audit_log.md) §A。
...
注：`Result<T, E>` 来自 `src/types/result.ts`（clawforum 通用 discriminated union / `{ ok: true; value }` 或 `{ ok: false; error }`）。
...
// 工厂（实然位置：`src/cli/commands/process-manager-factory.ts` createAgentProcessManager / 详 modules/l2_process_manager §A 登记）
