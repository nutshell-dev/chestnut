...
**应然**（2026-04-26 修订 / 跟 modules.md §11 align）：
- **装配归属**：按需（任何需要对目标目录做版本化快照的模块装）
- **资源**：目标目录内的 .git
- **依赖**：ProcessExec, FileSystem, AuditLog
- **耦合**：gitignore 模式聚合：条目的命名归各源模块（Stream / AuditLog / TaskSystem 等），Snapshot 通过参数接收，由 Assembly 装配时聚合
...
- ~~**A.6 — 基础设施越权做业务决策（修复方向已定，待实施）**~~ → **phase150+ 已实施**：`snapshot.ts:112` 返 `Result<void, ExpectedGitFailure>`；预期失败返回 discriminated result，不可预期失败抛 Error。
...
- ~~**A.7 — 跨模块资源名硬编码（修复方向已定，待实施）**~~ → **phase150+ 已实施**：`snapshot.ts:43` constructor 注入 `ignorePatterns: readonly string[]`；`ignore-patterns.ts` 消费侧定义。
...
### 7.C 原则对照（32 条，phase196 补齐）

对齐 phase187 L1 / phase193 L2 批量 backfill 模板（Module 11 + Design 11 / #1 展 4 面 + Philosophy 4 + Path 6）。合规一行按需；驱动/约束/灰度标黑。

#### Module Logic Principles（11 条）

- **M1 独立可变职责**：合规。单一职责 = agent 目录 git commit 历史序列化
- **M2 业务语义归属**：合规。`init` / `commit` / `addIgnorePattern` 由 Snapshot 发起；消费者（Runtime / Daemon / Motion）不代理 git 命令
- **M3 资源归属**：合规。`.git` 目录归 Snapshot 独占（§5 grep 核：字面量仅 `snapshot.ts` + `daemon.ts:182` 注释）
- **M4 持久化**：**驱动原则**。Snapshot 是"持久化一切 agent 状态"的实现者（每轮 commit 固化）
- **M5 依赖单向**：合规。L2 Snapshot → L1 FileSystem + L1 ProcessExec（内部）+ L2 AuditLog；无反向
- **M6 依赖结构稳定**：合规。constructor 4 参（dir / fs / audit / ignorePatterns）自 phase150+ 稳定
- **M7 耦合界面稳定**：合规。`Snapshot.init / commit / addIgnorePattern` + `createSnapshot` + `SNAPSHOT_IGNORE_PATTERNS` 三面长期稳定（**注**：§3 `CommitResult` 类型 drift 属文档层，非实际接口变化）
- **M8 耦合界面最小**：合规。class 3 方法 + 工厂 1 函数 + 常量 1；API 表面极小
- **M9 显式表达编译器可检**：**部分违反**。`dir: string` 既是 `FileSystem.baseDir` 又是 git cwd，两者必须一致的约束靠运行期校验（§5 登记），tsc 不可检；修复方向：引入 `AgentDir` branded type（未触发，观察）
- **M10 不合理停下**：未触发
- **M11 边界不对停下**：未触发

#### Design Principles（11 条 / #1 展 4 面 = 14）

- **D1a 信息不丢失**：合规。所有失败经 audit event 留痕（phase148 闭环）；A.6 重构后额外抛 Error(cause) 给消费者（已实施）
- **D1b 状态可观察**：合规。`SNAPSHOT_COMMITTED` / `SNAPSHOT_COMMIT_FAILED` / `SNAPSHOT_DEGRADED` 覆盖主要状态转移
- **D1c 中断可恢复**：**驱动原则**。Snapshot 是"中断可恢复"的核心实现（重启后 `recovery-snapshot` commit 固化 working tree 变更）
- **D1d 事后可审计**：**驱动原则**。`git log` + `audit.tsv` 双通道重建 agent 目录任一时刻状态
- **D2 不得丢弃/静默**：合规（phase148 + A.6 已实施重构闭环）
- **D3 用户可观察**：合规（audit event + `git log` 可查）
- **D4 LLM 调用恢复**：无关
- **D5 日志重建**：**驱动原则**。Snapshot + AuditLog 并列"单一事实源"实现者
- **D6 智能体决策主体**：无关（基础设施）
- **D7 系统可信路径**：合规。git 参数单引号包裹 + `'\\''` 转义（§5 / §C 核）防 shell 注入
- **D8 事件驱动**：合规
- **D9 多 claw 不隔绝**：灰度。每 agent 独立 `.git`；跨 agent 无 snapshot 聚合（Motion per-agent audit 隔离同构，B 类登记）
- **D10 motion 特殊**：合规（Motion 装配期用独立 `motionAudit`，B 类登记先例）
- **D11 CLI 唯一对外**：无关

#### Philosophy（4 条）

- **P1 Agent 即目录**：**驱动原则**。Snapshot 固化"agent 目录 = agent 状态"这一核心抽象的历史时间轴
- **P2 上下文工程**：合规（Snapshot 是"事后可重建 context"的最终保障）
- **P3 多 agent 利用**：合规（每 agent 独立 snapshot，互不干扰）
- **P4 系统为智能体服务**：合规（Snapshot 为 agent 失败/重启提供 safety net）

#### Path Principles（6 条）

- **Path #1 规划基于规划时刻事实**：✓ 契约 Read + 源码 150 行逐行核 → **发现 5 条契约↔实然 drift**（本 phase 核心贡献；§7.Phase 登记）
- **Path #2 差距显式登记**：✓ drift 清单全登记 §7.Phase；不隐藏
- **Path #3 语义一致最小变更单元**：✓ §7 内 APPEND 子节不改既有 A 类/B 类/C 类；drift 不就地改
- **Path #4 可回滚**：本地 only / 无破坏
- **Path #5 完成后复盘**：phase196 Step 4
- **Path #6 冲突立即中断**：触发 2 次（phase194 A 占 / phase195 C 占 → 切 196）

### 7.D 关键决策映射表（modules.md 迁移）

从 `design/modules.md` §关键设计决策章节迁移（2026-04-26 主会话；后续清理阶段重构）。原 KD 编号保留供对账。

- **KD#19（原 modules.md）Snapshot 轮级快照**：agent 目录的 git 快照在一轮执行结束后触发,不在每步触发,平衡历史可回滚与性能开销

---

### 7.Phase 执行纪律

#### phase196 纪律 — L2 Snapshot backfill + 契约 drift 捕获（2026-04-22，design 本地 only）

**scope**：承 phase187/192/193 APPEND 不解构模式；**形态变种**：既有 `## 7.` section 已占号 + 有 A 类/B 类/C 类 → 采用 §7 内 APPEND 子节（新 `### 7.C 32 条` + `### 7.Phase`）

**产出**：
- 既有 A 类 / B 类 / C 类 3 子节保留不改
- 新增 `### 7.C 原则对照（32 条）` + `### 7.Phase 执行纪律`（本节）
- 契约 +~100 行

**Path #1 契约↔实然 drift 5 条捕获**（独立契约修订 phase 候选）：

| # | 条目 | 契约当前声明 | 实际代码 | 修订方向 |
|---|---|---|---|---|
| 1 | A.6 commit 永不抛 | "修复方向已定，待实施" | `snapshot.ts:112-147` `Result<void, ExpectedGitFailure>` + throw rawErr（phase150+ 已实施）| 标"**已修复**"；更新 §失败语义表 + §3 接口示例 |
| 2 | A.7 跨模块资源名硬编码 | "修复方向已定，待实施" / "GITIGNORE_CONTENT 硬编码" | `ignore-patterns.ts` 消费侧定义 + constructor `ignorePatterns: readonly string[]` 注入（phase150+ 已实施）| 标"**已修复**"；§5 更新耦合描述；§6 常量名更新 |
| 3 | §3 `addIgnorePattern` 接口方法 | 登记为 `addIgnorePattern(pattern: string): void` 方法 | 实施为 constructor 第 4 参 `ignorePatterns: readonly string[]`（修复路径不同）| 更新 §3 接口签名；删 `addIgnorePattern` 方法描述 |
| 4 | §3 `CommitResult` type | `{ ok: true; sha: string } \| { ok: false; reason: CommitSkipReason }` | `Result<void, ExpectedGitFailure>`（无 sha 字段；error type 结构不同）| 更新 §3 类型签名；匹配 `Result<T, E>` + `ExpectedGitFailure` |
| 5 | §6 `GITIGNORE_CONTENT` 常量 | "定义在 snapshot.ts 顶部" | 已无此常量；改 `DEFAULT_IGNORES`（内部）+ 消费侧 `SNAPSHOT_IGNORE_PATTERNS` | 更新 §6 常量命名 + 删除过时描述 |

**不在本 phase 做**：drift 不就地改契约文本（scope 扩散风险；参 phase191 "决策越界提醒"）。修订建议 → **独立契约修订 phase**（编号待定，候选 phase20x）。

**升格候选触发**：

- **"Path #1 契约↔实然 drift 发现" 形态第 2-3 次验证**：
  - phase182 首次（B.p166-3 降档根因）
  - phase191 第 2 次（§7.A2 残余→零，契约滞后实然）
  - phase196 第 3 次（本 phase 5 条 drift 一次捕获）
  - **达 3 次阈值** → 可硬化到 `feedback_verify_facts_before_plan` 扩"backfill/登记类 phase 必对照实然代码 grep 核"

- **"§7 内 APPEND 子节"形态首次**（phase196 新形态）：
  - 适用场景：既有契约 `## 7.` section 已占 + 有 §A/§B/§C 子节
  - 动作：保留既有子节不改；在 §7 内追加 `### 7.C 32 条` + `### 7.Phase`
  - 与 phase192/193 "APPEND 全新 §7 节" 对照：编号已占时的子节 APPEND 变种

**对比定位**：
- 与 phase192（SessionStore "backfill 零新增"）：本 phase **非零新增**（新增 §7.C 32 条 + §7.Phase + 5 条 drift 登记）
- 与 phase193（Stream/FileWatcher/AuditLog APPEND 全新 §7）：本 phase **子节 APPEND** 形态变种
- 与 phase191（Daemon §7.A2 残余→零）：本 phase **5 条 drift 一次捕获**规模更大
- 与 phase182（B.p166-3 降档）：本 phase **仅登记不降档**（避免 scope 扩散）

**方法论贡献**：
1. "§7 内 APPEND 子节"形态首次（编号已占时的子节 APPEND 变种）
2. "契约 drift 一次批量登记 + 推迟独立修订 phase"模板（避免 backfill phase scope 扩散）
3. Path #1 drift 发现形态第 3 次验证达升格阈值

### 7.编号 drift 表

| modules.md 实然 § | 本契约 § | delta | 说明 |
|---|---|---|---|
| §11 | §7（含子节 7.A/7.B/7.C/7.D/7.Phase） | -4 | modules.md §11 = Snapshot / 本契约 §7 为内部 drift 子节结构（非 modules.md § 号） |
