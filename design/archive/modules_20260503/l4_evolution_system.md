...
- **M#5 依赖单向**：L4 → L1 (FileSystem) + L2 (AuditLog / SkillSystem.reload) + L4 同层 (TaskSystem 派 retro 子代理 / ContractSystem 经 callback subscription protocol Assembly wire / 0 直 import ContractSystem)（per arch §24 表 1 + 耦合列「订阅 contract_completed 事件协议」）/ 不上引 L6+
