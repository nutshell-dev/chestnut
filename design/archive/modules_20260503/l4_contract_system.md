...
- **M#5 依赖单向**：L4 → L1 (FileSystem) + L2 (AuditLog / Messaging) + L1 (ProcessExec / 脚本 acceptance) + L4 同层 (TaskSystem / 直 dep 调度 verifier 子代理) + L2 (ToolProtocol)（per arch §22 表 1）/ 不反向依赖 Runtime / Daemon / 5 直 import 跨层历史已闭环
