...
| 资源 | 类别 | 持久化 |
|---|---|---|
| `<agentDir>/.git` | 版本化仓库（独占）| ✓ |
| `<agentDir>/.gitignore` | 装配期写入的 ignore 清单（独占 / 条目由 Assembly 聚合注入）| ✓ |
| `consecutiveFailures` | 运行期 per-instance 内存状态 | ✗ 重启即丢 / 但磁盘 commit 序列保留 |

**目标目录版本化历史状态** — clawforum 内部任何目录版本化必经 Snapshot 间接访问 / 是「版本化历史 artifact」（实现为 `.git` + `.gitignore`）唯一调用入口。

> 注：M#9 跨模块 ignore 聚合显式经参数注入（条目由 Assembly 聚合 / 本模块不内化字面量 / 实施细节归 §1.做）。
...
| A.tryCleanupGit-rethrow | drift | **✅ closed（phase 636 Step B.1 / main `4a467299`）** | `init()` 路径 cleanup 失败原 line 111 `throw cleanupErr` 冒泡 / `Result<void, ExpectedGitFailure>` API 契约违约（caller 期 `errResult(failure)` 实见 thrown error）/ 与 commit() syncDir cleanup（line 138-148 audit-only / 不 throw）不对称。**phase 636 落地**：删 `throw cleanupErr` + 注释扩 best-effort cleanup invariant / mirror commit syncDir 模板 / 0 NEW const（复用 INIT_CLEANUP_FAILED）/ NEW test「init returns Result.err even when cleanup itself fails」/ 反向 3/3 PASS |
...
| `consecutiveFailures` per-instance 状态分裂 | 实例局部字段 | **升 §A 必修**（design verify 真 deep 后升档 / r60+ Meta 33 候选）/ 应然单实例约束 vs 实然多实例化 真 contradiction / 修复路径推 r51+ design phase：(a) per-dir 模块级 Map 聚合（共享失败计数）/ (b) per-dir 单例工厂（防多实例化）/ Assembly 装配期保证 single instance per dir / **phase 718 hygiene re-open**：早期 strikethrough 与 src per-instance + 升档条「真 contradiction」自相矛盾 / strikethrough 误标 resolved → 去除恢复 open 状态 |
...
| **L2a.G1 (snapshot)** ctor 不显式注入 ProcessExec | **业务决策性 design-gap / r65 起 cross-doc audit 浮出**：interfaces/l2a.md ctor `(dir, fs, audit, ignorePatterns)` / arch 表 1 Snapshot row 依赖列「ProcessExec、FileSystem、AuditLog」/ git exec 通过模块内部直接调用 L1 ProcessExec（capability+direct 默认 ok）/ 但 interfaces 未在 ctor 注释或使用语义说明 ProcessExec direct 调用方式 / derive 链断 | **业务决策性 / 用户拍板候选**：α interfaces ctor 注释加一句「git exec 通过直接调用 L1 ProcessExec / 不经 ctor DI（同 L1 capability+direct 默认）」/ β 改 ctor 显式注入 ProcessExec（强 DI 模式）/ γ 保留现状（L1 capability+direct 默认 / interfaces 无需显式）|
...
- **M#1 独立可变职责**：单一职责 = agent 目录 git commit 历史序列化
- **M#2 业务语义归属**：init / commit / 失败处理由 Snapshot 发起 / 消费者不代理 git 命令
- **M#3 资源归属**：`.git` 目录归 Snapshot 独占 / grep 核字面量仅 snapshot.ts
- **M#4 持久化**：**驱动原则**（每轮 commit 固化 / 是「持久化一切 agent 状态」实现者）
- **M#5 依赖单向**：Snapshot → L1 FileSystem + L1 ProcessExec + L2 AuditLog（per arch §7 表 1）/ 0 反向
- **M#6 依赖结构稳定**：constructor 4 参（dir / fs / audit / ignorePatterns）/ phase150+ 稳定
- **M#7 耦合界面稳定**：Snapshot 3 方法 + createSnapshot 工厂 + SNAPSHOT_IGNORE_PATTERNS 常量
- **M#8 耦合界面最小**：API 表面极小（init / commit / 工厂）
- **M#9 显式表达编译器可检**：`dir: string` 既是 baseDir 又是 git cwd / 两者一致约束靠运行期校验（tsc 不可检 / 修复方向：`AgentDir` branded type / 未触发观察）
- **M#10-M#11**：未触发
...
- **D8 事件驱动**：受信组件
- **D9 CLI 唯一外部入口**：N/A（本模块 L2 内部基础服务 / 0 外部入口 / CLI 命令调 createSnapshot 工厂消费）
- **D10 多 claw 不隔绝**：灰度（每 agent 独立 .git / 跨 agent 无 snapshot 聚合）
- **D11 motion 特殊**：Motion 装配期用独立 motionAudit（B 类登记先例）
- **D4 / D6**：无关
...
- 2026-05-04 / cross-doc audit drift 修订（Module Logic 命名 M1-M11 → M#1-M#11 / Design Principles D9-D11 verbatim 编号修正 align principles.md / §3 资源改 table 「.git + .gitignore + consecutiveFailures」+ 注脚 align 其他模块 / 注：§7.C P3 verbatim 已正确）
- 2026-05-05 / phase 485 Snapshot ctor +syncDir + commit success 后 generic clean syncDir 落地（main `9b703d7e` / merge `5e687e65` / Cluster A r+1 收官 第 4 phase / cross-ref l2_command_tool.md §A.7）/ Snapshot ctor +`syncDir: string` 字段（装配期 commit hook 触发 generic clean syncDir / cleanup 失败 best-effort 不抛 + audit `SNAPSHOT_SYNC_CLEAN_FAILED`）/ SNAPSHOT_IGNORE_PATTERNS 加 `tasks/sync` 防 commit 循环（git 不追踪 sync 副产物）/ commit success 后 clean 实施 / **Cluster A 4/4 全闭**（phase 481+482+483+485）/ **3 原则全 align**：M#3 资源唯一归属（syncDir 装配级共享 / 不归 AsyncTaskSystem own）+ M#1 副产品归本模块（Snapshot 自身清）+ M#5 防反向注入 handler（强排除 Assembly handler 注入选项）
- 2026-05-05 / r65 cross-doc audit 单 doc 一致性核（modules/l2_snapshot.md vs arch §7 + 表 1/2 + interfaces/l2a.md Snapshot 节）/ 0 derive drift / 主 derive 全 align（M#1-M#11 + Design Principle **D1a-d 驱动**+D2/**D3/D5 驱动**+D7/D11 + D8/D10 灰度 + D4/D6 无关 + D9 N/A + Philosophy **P1 驱动原则**「Snapshot 固化 agent 目录 = agent 状态」+P2+P3+P4 + Path #1-#7）/ 3 主能力 align arch 表 2 / 3 dep + 3 caller + ignorePatterns 注入耦合 align arch 表 1 / 资源 .git+.gitignore align arch 表 1 / 补 phase 485 closure timeline entry / L2a.G1 (snapshot) ctor 不显式注入 ProcessExec design-gap 已登记 §B（业务决策性 α/β/γ 候选）/ design only / 0 src 改
