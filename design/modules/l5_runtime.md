...
| ~~**A.dead-field-cleanup `maxConcurrentTasks` + `subagentMaxSteps` dead fields**~~ | drift | **✅ closed by phase 683 Step E.1+E.2** | `RuntimeOptions` 删 2 dead fields（`maxConcurrentTasks?: number` + `subagentMaxSteps?: number`）/ `runtime.ts:114` 删 `maxConcurrentTasks` default + import cleanup + `assemble.ts:469-470` 删 2 字段传参 / `DEFAULT_MAX_CONCURRENT_TASKS` 仍活（AsyncTaskSystem + config + CLI 真用）/ `subagentMaxSteps` 真活保留（SubAgent/ToolExecutor/ExecContextImpl 路径）/ 6 files / -8 行 / 0 行为差 / `tsc+test` 全绿 |
